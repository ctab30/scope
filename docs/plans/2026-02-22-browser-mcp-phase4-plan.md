...
**Architecture:** Same IPC pattern as Phases 1-3: ScopeMCP inserts commands into SQLite `browserCommands` table, GUI polls and executes via `callAsyncJavaScript` on WKWebView in `.defaultClient` content world. Safety features are middleware in `BrowserCommandExecutor` — domain allowlist checks before navigation, sanitization after snapshot/extract returns.
...
Run: `cd /Users/connor/Documents/claude-code-projects/scope && swift build 2>&1 | tail -5`
...
case .domainNotAllowed(let domain):
    return "Domain '\(domain)' is not in the allowed list. Configure allowed domains in Scope.app settings."
```
...
### Task 5: ScopeMCP Tool Definitions and Wrappers

Add 4 tool definitions, 4 dispatch cases, and 4 wrapper functions to ScopeMCP/main.swift.
...
[
    "name": "browser_upload",
    "description": "Set a file on an <input type='file'> element. Reads the file from disk, encodes it, and assigns it to the input. Triggers change and input events. Requires Scope.app to be running with the browser tab visible.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    "name": "browser_drag",
    "description": "Drag an element to a target element using HTML5 drag and drop events. Dispatches the full drag event sequence: dragstart, drag, dragenter, dragover, drop, dragend. Requires Scope.app to be running with the browser tab visible.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    "name": "browser_iframe",
    "description": "Switch execution context to an iframe for subsequent commands (snapshot, click, type, etc.), or back to the main frame. Call with a ref to enter an iframe, or without ref to return to main frame. Only same-origin iframes are accessible. Use browser_snapshot to see available iframes. Requires Scope.app to be running with the browser tab visible.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "Clear browsing data (cookies, cache, localStorage). Useful for resetting login state, clearing cached data, or testing fresh page loads. Clears all data by default. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
            "types": [
...
Build the complete project, rebuild the ScopeMCP binary, and do a final review.
...
Run: `cd /Users/connor/Documents/claude-code-projects/scope && swift build 2>&1 | tail -10`
Expected: Build Succeeded

**Step 2: Rebuild ScopeMCP binary**

Run: `cd /Users/connor/Documents/claude-code-projects/scope && swift build --product ScopeMCP 2>&1 | tail -5`
Expected: Build Succeeded
...
```bash
grep -c '"name": "browser_' Context/Sources/ScopeMCP/main.swift
```
...
- [ ] All 4 tool definitions in ScopeMCP with correct inputSchema
