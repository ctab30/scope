...
**Architecture:** Same SQLite IPC pattern as Phase 1 & 2. ScopeMCP inserts command rows, GUI app polls and executes JS via `callAsyncJavaScript` in `.defaultClient` content world, writes results back. No schema changes, no new dependencies.
...
### Task 5: Add 3 tool definitions and wrappers to ScopeMCP
...
```swift
            ...
            [
                "name": "browser_press",
                "description": "Press a key or key combination. Targets a specific element by ref, or the currently focused element if no ref is provided. Handles Enter (submits forms), Tab (moves focus), Escape, arrow keys, and any single character. Requires Scope.app to be running with the browser tab visible.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
            ...
            [
                "name": "browser_eval",
                "description": "Execute JavaScript on the page and return the result. The expression runs inside an async function body, so use 'return' to return values and 'await' for promises. Use for reading page state, calling APIs, or handling edge cases other tools can't cover. Requires Scope.app to be running with the browser tab visible.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
            ...
            [
                "name": "browser_hover",
                "description": "Hover over an element by ref. Dispatches mouseenter and mouseover events. Useful for dropdown menus, tooltips, and hover-state UI that requires mouse presence. Scrolls element into view first. Requires Scope.app to be running with the browser tab visible.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
...
git commit -m "feat: add Phase 3 tool definitions and wrappers to ScopeMCP"
```
...
**Prerequisites:**
- Scope.app must be running with a browser tab visible
- The ScopeMCP binary must be rebuilt: `cd /Users/connor/Documents/claude-code-projects/scope/Context && swift build`
