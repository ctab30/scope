...
**Goal:** Add 9 read-only browser MCP tools to ScopeMCP, enabling Claude Code to navigate, read, and inspect pages in Context's built-in WKWebView browser.

**Architecture:** ScopeMCP writes browser commands to a shared SQLite table. The GUI app observes new rows via GRDB ValueObservation, executes JavaScript on the WKWebView, and writes results back. ScopeMCP polls for results with a 50ms interval and 15s timeout.
...
### Task 2: BrowserCommand Model in ScopeMCP

**Files:**
- Modify: `Context/Sources/ScopeMCP/main.swift:88` (after the `Client` struct, before `// MARK: - MCP Protocol Types`)
...
func executeBrowserCommand(tool: String, args: [String: Any] = [:], timeout: TimeInterval = 5.0) throws -> String {
    ...
    throw MCPError(message: "Browser command timed out after \(Int(timeout))s. Is Scope.app running with the browser tab visible?")
}
```
...
### Task 3: Browser Tool Definitions and Handlers in ScopeMCP
...
[
    ...
    "description": "Navigate the browser to a URL. Opens a new tab if none are open. Waits for page load to complete. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "Get the accessibility tree of the current page as compact structured text. Returns ARIA roles, labels, and interactive element refs. This is the primary tool for understanding page content and structure. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "Extract text content from a page element using a CSS selector. Returns the text content of the first matching element. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "List all open browser tabs with their URLs, titles, and loading state. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
...
[
    ...
    "description": "Get JavaScript console log entries (log, warn, error, info) from a browser tab. Useful for debugging web applications. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
            "tab_id": ["type": "string", "description": "Tab ID (defaults to active tab)"],
...
[
    ...
    "description": "Take a PNG screenshot of the current page. Returns the file path so you can read the image. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
            "tab_id": ["type": "string", "description": "Tab ID (defaults to active tab)"]
...
[
    ...
    "description": "Open a new browser tab. Optionally navigate to a URL. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "Close a browser tab by its ID. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
[
    ...
    "description": "Switch the active browser tab to the specified tab. Requires Scope.app to be running.",
    "inputSchema": [
        "type": "object",
        "properties": [
...
/// Observes the browserCommands table for pending commands from ScopeMCP,
...
enum BrowserCommandError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noBrowser:
            return "Browser is not available. Make sure a project window with the browser tab is open in Scope.app."
        case .noActiveTab:
...
Run the ScopeMCP binary and send an initialize + tools/list request:

```bash
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}
{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}' | /Users/connor/Documents/claude-code-projects/scope/Context/.build/debug/ScopeMCP 2>/dev/null
```
...
Launch Scope.app (or run the GUI binary). Check that the `browserCommands` table exists:
...
With Scope.app running and a browser tab open, send a command via the MCP binary:

```bash
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}
{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"browser_list_tabs","arguments":{}}}' | /Users/connor/Documents/claude-code-projects/scope/Context/.build/debug/ScopeMCP 2>/dev/null
```
...
| `Context/Sources/ScopeMCP/main.swift` | Modify | +200 |
