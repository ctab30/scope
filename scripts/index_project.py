...
to populate the scope.db with real embeddings.
...
DB_PATH = os.path.expanduser("~/Library/Application Support/Scope/scope.db")
...
def get_api_key():
    result = subprocess.run(
        ["plutil", "-extract", "openRouterAPIKey", "raw",
         os.path.expanduser("~/Library/Preferences/com.scope.app.plist")],
        capture_output=True, text=True
    )
    key = result.stdout.strip()
    if not key:
        print("ERROR: No OpenRouter API key found.")
        print("Set it in Scope > Settings > Context Engine.")
        sys.exit(1)
    return key
...
def main():
    ...
    print("=" * 60)
    print("Scope Engine — Full Project Indexer")
    print("=" * 60)
