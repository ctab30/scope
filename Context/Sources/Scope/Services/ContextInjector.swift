...
/// Manages the integration surface between Scope and Claude Code.
...
class ContextInjector {
    ...
    /// Markers used to identify the Scope-managed section in CLAUDE.md
    static let sectionStart = "<!-- Scope managed section -->"
    static let sectionEnd = "<!-- End Scope section -->"

    /// The content injected between the markers
    private static let managedContent = """
    # Scope
    This project uses Scope for session memory.
    Use the `scope` MCP tools to retrieve project history,
    ...
    /// Add or update the Scope managed section in the project's CLAUDE.md.
    ...
    /// Remove the Scope managed section from the project's CLAUDE.md.
    ...
    /// Creates ~/Library/Application Support/Scope/mcp-config.json
    ///
    ...
    func configureMCPConnection() throws {
        ...
        let config: [String: Any] = [
            "mcpServers": [
                "scope": [
                    "command": "scope-mcp-bridge",
                    "args": [] as [String],
                    "description": "Scope - Session memory and project intelligence for Claude Code"
                ]
            ]
        ]
    ...
    /// The deployed ScopeMCP binary path.
    static var mcpBinaryPath: String {
        let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory, in: .userDomainMask
        ).first!
        return appSupport
            .appendingPathComponent("Scope/bin/ScopeMCP")
            .path
    }
    ...
    func installMCP(for cli: CLIProvider, projectPath: String) throws -> String {
        ...
        guard fileManager.fileExists(atPath: binaryPath) else {
            throw InjectorError.fileOperationFailed("ScopeMCP binary not found at \(binaryPath)")
        }
    ...
    /// Write the Scope managed section to the appropriate instruction file for this CLI.
