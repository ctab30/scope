...
/// Observes the browserCommands table for pending commands from WorkspaceMCP,
...
class BrowserCommandExecutor: ObservableObject {
    ...
    private func startPolling() {
        // Poll every 50ms for pending commands from WorkspaceMCP (matches MCP-side initial poll rate)
...
enum BrowserCommandError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noBrowser:
            return "Browser is not available. Make sure a project window with the browser tab is open in Workspace."
        case .noActiveTab:
