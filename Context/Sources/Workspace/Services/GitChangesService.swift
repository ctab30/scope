...
class GitChangesService: ObservableObject {
    ...
    func push(setUpstream: Bool = false) async -> (success: Bool, message: String) {
        ...
        if result.1 == 0 {
            await refresh()
            return (true, result.0?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "Pushed successfully")
        } else {
            return (false, result.0?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "Push failed")
        }
    }

    /// Checks if the current branch has an upstream tracking branch.
    nonisolated static func hasUpstream(at path: String, branch: String) -> Bool {
        let result = runGit(["config", "--get", "branch.\(branch).remote"], at: path)
        return result != nil && !result!.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }

    /// Returns the number of commits ahead of the upstream (0 if up-to-date or no upstream).
    nonisolated static func commitsAhead(at path: String) -> Int {
        guard let output = runGit(["rev-list", "--count", "@{u}..HEAD"], at: path) else { return 0 }
