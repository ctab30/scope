...
struct ChatMessageView: View {
    ...
    private var userBubble: some View {
        VStack(alignment: .trailing, spacing: WorkspaceTheme.Spacing.xxxs) {
            Text(message.content)
                ...
                .background(
                    RoundedRectangle(cornerRadius: WorkspaceTheme.Radius.medium, style: .continuous)
    ...
    private var assistantBubble: some View {
        VStack(alignment: .leading, spacing: WorkspaceTheme.Spacing.xxs) {
            MarkdownContentView(content: message.content)
                ...
                .padding(.vertical, WorkspaceTheme.Spacing.sm)
                .background(
                    WorkspaceTheme.Colors.controlBg,
                    in: RoundedRectangle(cornerRadius: WorkspaceTheme.Radius.small, style: .continuous)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: WorkspaceTheme.Radius.small, style: .continuous)
                        .strokeBorder(WorkspaceTheme.Colors.separator.opacity(WorkspaceTheme.Opacity.subtleBorder), lineWidth: 0.5)
                )
...
private struct MarkdownContentView: View {
    ...
    private func blockView(_ block: Block) -> some View {
        ...
        case .listItem(let text):
            HStack(alignment: .firstTextBaseline, spacing: WorkspaceTheme.Spacing.xs) {
                Text("\u{2022}")
                    .font(WorkspaceTheme.Font.body)
            ...
            .padding(.leading, WorkspaceTheme.Spacing.xxs)
        ...
        case .numberedItem(let number, let text):
            HStack(alignment: .firstTextBaseline, spacing: WorkspaceTheme.Spacing.xs) {
                Text("\(number).")
                    .font(WorkspaceTheme.Font.body)
                    .foregroundColor(.secondary)
                    .frame(minWidth: WorkspaceTheme.Spacing.lg, alignment: .trailing)
                inlineText(text)
                    .font(WorkspaceTheme.Font.body)
            }
            .padding(.leading, WorkspaceTheme.Spacing.xxs)
        ...
        case .blockquote(let text):
            HStack(spacing: 0) {
                ...
                inlineText(text)
                    ...
                    .padding(.leading, WorkspaceTheme.Spacing.sm)
            }
            ...
            Text(code)
                ...
                .background(
                    RoundedRectangle(cornerRadius: WorkspaceTheme.Radius.small)
                        .fill(WorkspaceTheme.Colors.textBg.opacity(0.5))
                )
                .overlay(
                    RoundedRectangle(cornerRadius: WorkspaceTheme.Radius.small)
