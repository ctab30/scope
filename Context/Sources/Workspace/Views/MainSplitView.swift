...
/// Disables native fullscreen so the green button performs zoom (fill screen) instead.
struct TransparentWindowSetter: NSViewRepresentable {
    ...
    /// Must be called after the window delegate is set, as the delegate handles blocking fullscreen.
    private static func retargetZoomButton(_ window: NSWindow) {
