...
struct WorkspaceApp: App {
    ...
    init() {
        ...
        // Migrate data from old paths before any DB/service init
        Self.migrateFromContext()
        Self.migrateFromScopeToWorkspace()
        ...
        // Deploy MCP binary to ~/Library/Application Support/Workspace/bin/
    ...
    private static func deployMCPBinary() {
        ...
        let bundleMCP = execURL.deletingLastPathComponent().appendingPathComponent("WorkspaceMCP")
        ...
        let dest = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
            .first!.appendingPathComponent("Workspace/bin", isDirectory: true)
        let destBinary = dest.appendingPathComponent("WorkspaceMCP")
    ...
    /// Migrates existing user data from old "Context" paths to new "Workspace" paths.
    ...
    private static func migrateFromContext() {
        ...
        let newDir = appSupport.appendingPathComponent("Workspace", isDirectory: true)
        ...
        let dbRenames = [
            ("context.db", "workspace.db"),
            ("context.db-wal", "workspace.db-wal"),
            ("context.db-shm", "workspace.db-shm"),
        ]
    ...
    /// Migrates user data from "Scope" paths to "Workspace" paths.
    /// Moves Application Support directory, renames database files, and moves hooks dir.
    private static func migrateFromScopeToWorkspace() {
        let fm = FileManager.default
        let appSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!

        let oldDir = appSupport.appendingPathComponent("Scope", isDirectory: true)
        let newDir = appSupport.appendingPathComponent("Workspace", isDirectory: true)
        ...
        if fm.fileExists(atPath: oldDir.path), !fm.fileExists(atPath: newDir.path) {
            ...
            // Rename database files inside
            let dbRenames = [
                ("scope.db", "workspace.db"),
                ("scope.db-wal", "workspace.db-wal"),
                ("scope.db-shm", "workspace.db-shm"),
            ]
            for (oldName, newName) in dbRenames {
                let oldFile = newDir.appendingPathComponent(oldName)
                let newFile = newDir.appendingPathComponent(newName)
        ...
        // Migrate hooks dir ~/.scope/ → ~/.workspace/
        let homeDir = fm.homeDirectoryForCurrentUser
        let oldHooksDir = homeDir.appendingPathComponent(".scope", isDirectory: true)
        let newHooksDir = homeDir.appendingPathComponent(".workspace", isDirectory: true)
        if fm.fileExists(atPath: oldHooksDir.path), !fm.fileExists(atPath: newHooksDir.path) {
            try? fm.moveItem(at: oldHooksDir, to: newHooksDir)
        }
    }
                ...
                .sheet(isPresented: $showDeepLinkSheet) {
                    VStack(spacing: 16) {
                        if case .success(let cli) = deepLinkResult {
                            ...
                            Text("MCP Configured")
                                .font(.headline)
                            Text("Workspace is now connected to \(cli.displayName).")
    ...
    private func handleDeepLink(_ url: URL) {
        ...
        do {
            ...
            process.arguments = ["claude", "mcp", "add", "workspace", binaryPath]
